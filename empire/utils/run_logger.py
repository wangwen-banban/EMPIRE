...
# RunLogger Default Format String
...
class DistributedRunLogger:
    def __init__(self, name: str) -> None:
        """Initializer for an RunLogger object that wraps logging & `accelerate.PartialState`."""
...
class PureRunLogger:
    def __init__(self, name: str) -> None:
        """Initializer for an RunLogger object that just wraps logging."""
...
def initialize_run_logger(name: str) -> Union[DistributedRunLogger, PureRunLogger]:
    return DistributedRunLogger(name) if int(os.environ.get("WORLD_SIZE", -1)) != -1 else PureRunLogger(name)
