...
@lru_cache(maxsize=1)
def _pose_path_3d_class():
    """Load evo only when trajectory alignment is requested."""
    try:
        from evo.core.trajectory import PosePath3D
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "Pose alignment requires evo; install the 'depth' or 'public' extra."
        ) from exc
    return PosePath3D
...
def _umeyama_sim3_from_paths(pose_ref, pose_est):
    pose_path_3d = _pose_path_3d_class()
    path_ref = pose_path_3d(poses_se3=pose_ref.copy())
    path_est = pose_path_3d(poses_se3=pose_est.copy())
