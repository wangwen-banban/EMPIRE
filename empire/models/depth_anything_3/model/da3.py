...
class DepthAnything3Net(nn.Module):
    ...
    def forward(
        ...
        output.aux_cls = self._extract_auxiliary_features_cls(aux_feats, export_feat_layers, H, W)
    ...
    def _extract_auxiliary_features(
        ...
        if feats[0].shape[-2] != (H // self.PATCH_SIZE) * (W // self.PATCH_SIZE):
            feats = [out[..., 1 :, :] for out in feats]
        aux_features = Dict()
    ...
    def _extract_auxiliary_features_cls(
        self, feats: list[torch.Tensor], feat_layers: list[int], H: int, W: int
    ) -> Dict[str, torch.Tensor]:
        """Extract auxiliary features from specified layers."""
        aux_features = Dict()
        ...
        for feat, feat_layer in zip(feats, feat_layers):
            ...
            aux_features[f"feat_layer_{feat_layer}_cls"] = feat_reshaped
